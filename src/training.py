...
def get_scores(y_true, y_pred, y_pred_proba):
    return {
        ...
        "ROC AUC": roc_auc_score(y_true, y_pred_proba, average="weighted", multi_class="ovr")}
...
def training(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("Balanced Accuracy", scores["Balanced Accuracy"])
        mlflow.log_param("F1 Score", scores["F1 Score"])
        mlflow.log_param("Precision", scores["Precision"])
        mlflow.log_param("Recall", scores["Recall"])
        mlflow.log_param("ROC AUC", scores["ROC AUC"])
        
        mlflow.sklearn.log_model(
            sk_model=xgb_clf,
            artifact_path="xgboost_model",
            registered_model_name="MyXGBClassifier",
            signature=mlflow.models.infer_signature(X_train, xgb_clf.predict(X_train)), # Para XGBoost, usa dtrain para inferir la firma
            input_example=X_train[0:2]
        )

        mlflow.log_artifact(model_folder + "scaler.pkl", artifact_path="preprocessing")
        mlflow.log_artifact(model_folder + "le.pkl", artifact_path="preprocessing")

        os.remove(model_folder + "scaler.pkl")
        os.remove(model_folder + "le.pkl")
    return xgb_clf

def generate_hybrid_id():
    timestamp_part = datetime.now().strftime("%Y%m%d_%H%M%S")
    unique_part = hashlib.sha256(str(datetime.now()).encode()).hexdigest()[:8] 
...
def intrusion_pipeline():
    ...
    model = training(X_train, y_train, X_val, y_val, X_test, y_test)
    return
